...
function changestatus (dataName)
	...
	if dataName~="status" then return end
	local status=getElementData(source,dataName)
	if (status~="stasis" ) then
...
function setBotFollow(ped, leader,sprint)
	if (isElement(ped)) and (isElement(leader)) then
